        ...
        for farm_id in active_lift['farm_ids']:
            ...
            for schedule_item in schedule:
                dates_est = [
                    ...
                    for date in schedule_item.get("dates", [])
                ]
                ...
                new_schedule = LiftActiveSchedule(
                    farm_id=ObjectId(farm_id), schedule=new_lift_command
                )
                new_schedule.save()
                ...
                if eastern_time:
                    lift_response['created_at'] = eastern_time.strftime(
                        "%Y-%m-%d %H:%M:%S EST"
                    )
                        ...
                        for dt in command['dates']:
                            ...
                            if eastern_date:
                                converted_dates.append(
                                    eastern_date.strftime("%Y-%m-%d %H:%M:%S EST")
                                )
        ...
        for farm_id in archive_lift['farm_ids']:
            ...
            for schedule_item in schedule:
                ...
                new_schedule = LiftArchiveSchedule(
                    farm_id=ObjectId(farm_id), archived_schedules=new_lift_command
                )
                new_schedule.save()
                ...
                if eastern_time:
                    lift_response['archived_at'] = eastern_time.strftime(
                        "%Y-%m-%d %H:%M:%S EST"
                    )
                ...
                if "lift_end_time" in command and command['lift_end_time']:
                    command['lift_end_time'] = convert_to_eastern_time(
                        command['lift_end_time']
...
def get_active_lift_schedule(id):
    try:
        ...
        if data:
            ...
            if eastern_time:
                active_data['created_at'] = eastern_time.strftime(
                    "%Y-%m-%d %H:%M:%S EST"
                )
            ...
            for schedule in active_data.get("schedule", []):
                ...
                if "lift_end_time" in schedule:
                    eastern_end_time = convert_to_eastern_time(
                        schedule['lift_end_time']
                    )
                    if eastern_end_time:
                        schedule['lift_end_time'] = eastern_end_time.strftime(
                            "%Y-%m-%d %H:%M:%S EST"
                        )
...
def get_mult_active_lifts(id):
    try:
        ...
        logging.info(
            f"UTC dates: start date = {start_date_utc} and end date = {end_date_utc}"
        )
        ...
        for lift in all_lifts:
            ...
            if match_found:
                ...
                eastern_time = convert_to_eastern_time(lift.created_at)
                if eastern_time:
                    lift_data['created_at'] = eastern_time.strftime(
                        "%Y-%m-%d %H:%M:%S EST"
                    )
                ...
                for cmd in lift_data.get("schedule", []):
                    if "dates" in cmd:
                        ...
                        for dt in cmd['dates']:
                            ...
                            if eastern_date:
                                converted_dates.append(
                                    eastern_date.strftime("%a, %d %b %Y %H:%M:%S EST")
                                )
...
def get_archive_lift_schedule(id):
    try:
        ...
        if data:
            ...
            if eastern_time:
                archive_data['archived_at'] = eastern_time.strftime(
                    "%Y-%m-%d %H:%M:%S EST"
                )
            ...
            for schedule in archive_data.get("archived_schedules", []):
                if "lift_end_time" in schedule:
                    ...
                    if eastern_end:
                        schedule['lift_end_time'] = eastern_end.strftime(
                            "%Y-%m-%d %H:%M:%S EST"
                        )
    ...
    try:
        ...
        for lift in lift_list:
            ...
            for schedule in lift.get("archived_schedules", []):
                if "lift_end_time" in schedule:
                    eastern_lift_end = convert_to_eastern_time(
                        schedule['lift_end_time']
                    )
                    if eastern_lift_end:
                        schedule['lift_end_time'] = eastern_lift_end.strftime(
                            "%Y-%m-%d %H:%M:%S EST"
                        )
    ...
    try:
        ...
        for lift in all_lifts:
            ...
            if match_found:
                ...
                eastern_time = convert_to_eastern_time(lift.archived_at)
                if eastern_time:
                    lift_data['archived_at'] = eastern_time.strftime(
                        "%Y-%m-%d %H:%M:%S EST"
                    )
                ...
                for cmd in lift_data.get("archived_schedules", []):
                    ...
                    if "lift_end_time" in cmd:
                        ...
                        if eastern_end_time:
                            cmd['lift_end_time'] = eastern_end_time.strftime(
                                "%Y-%m-%d %H:%M:%S EST"
                            )
                            logging.info(
                                f"Lift end time in EST: {cmd['lift_end_time']}"
                            )
