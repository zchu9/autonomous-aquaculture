...
#define RENOGY_CONFIG SERIAL_8E1 // modbus *requires* even parity or 2 stop bits. other implementations don't include this (?)
#define READREG  0x03
#define RENOGY_DELAY 0

struct renogyDevice {
    uint8_t deviceAddress;
};

int genCRC(unsigned char buffer[], size_t buffer_size) {
    // generate a checksum for a given message. 
    // follows CRC16-Modbus convention; but renogy devices send the bytes reversed for CRC (low, then high);
    int value = 0xFFFF;
...
bool crcCheck(int checksum, unsigned char buffer[], size_t buffer_size) {
    int value = genCRC(buffer, buffer_size);
    if (value == checksum) {
        return true;
    }
    return false;
